...
def filter_low_correction_content(query: str, docs: List[Document], llm: BaseChatModel) -> List[Document]:
    ...
    try:
        ...
        logger.error(f"filter_low_correction_content 发生异常: {e}, 返回原始文档")
        generate_time = time.time() - start_time
        logger.info(f"  filter_low_correction_content: {{'generate_time': {generate_time:.3f}, 'docs_count': {len(docs)}, 'error': '使用原始文档'}}")
        return docs
...
def filter_low_correction_doc_llm(query: str, docs: List[Document], llm: BaseChatModel) -> List[Document]:
    ...
    try:
        ...
        logger.error(f"filter_low_correction_doc_llm 发生异常: {e}, 返回原始文档")
        generate_time = time.time() - start_time
        logger.info(f"  filter_low_correction_doc_llm: {{'generate_time': {generate_time:.3f}, 'docs_count': {len(docs)}, 'error': '使用原始文档'}}")
        return docs
...
def filter_low_correction_doc_embeddings(query: str, docs: List[Document],
    ...
    try:
        ...
        logger.error(f"filter_low_correction_doc_embeddings 发生异常: {e}, 返回原始文档")
        return docs
...
def filter_redundant_doc_embeddings(query: str, docs: List[Document],
    ...
    try:
        ...
        logger.error(f"filter_redundant_doc_embeddings 发生异常: {e}, 返回原始文档")
        return docs
