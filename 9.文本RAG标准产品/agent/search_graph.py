...
class SearchGraph:
    ...
    def __init__(self, config: AppConfig, power_model: BaseChatModel) -> None:
        ...
        # 创建数据库检索工具
        db_tool, db_llm, db_node = create_db_search_tool(config, power_model)
        self.db_search_tool = db_tool
        self.db_search_llm = db_llm
        self.db_tool_node = db_node

        # 创建网络搜索工具(如果未启动，均赋值为: None)
        web_tool, web_llm, web_node = create_web_search_tool(config, power_model)
        self.network_search_tool = web_tool
        self.network_search_llm = web_llm
        self.network_tool_node = web_node

        # 创建知识图谱检索工具
        kgraph_tool, kgraph_llm, kgraph_node = create_kgraph_search_tool(config, power_model)
