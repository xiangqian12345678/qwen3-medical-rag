...
try:
    ...
    from ..utils import del_think, format_document_str, json_to_list_document, _should_call_tool
    from .web_searcher import get_ws
except ImportError:
    # 回退到直接导入（当直接运行文件时）
    from prompts.templates import get_prompt_template
    from config.models import AppConfig
    from agent.utils import del_think, format_document_str, json_to_list_document, _should_call_tool
    from search.web_searcher import get_ws
